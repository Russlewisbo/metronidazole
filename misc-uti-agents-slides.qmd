...
::: {.notes}
This is a "miscellaneous agents" session, but there is a single unifying idea: for all four drugs, pharmacology — not spectrum — decides the clinical fate. Metronidazole's near-perfect absorption became a liability in luminal gut infection; nitrofurantoin and fosfomycin work in cystitis precisely because they concentrate in urine and nowhere useful else; methenamine only becomes a drug at all after chemical conversion in acidic bladder urine. Keep that lens on throughout.
:::
...
::: {.notes}
Frame the arc. Metronidazole and the UTI agents are grouped in the textbook as "miscellaneous," but they pair naturally: each shows a PK property driving the clinical story. For metronidazole, complete absorption is the problem in the gut lumen. For the urinary agents, urinary concentration is the whole point. This is a useful teaching contrast to run through the session.
:::
...
::: {.notes}
The serendipity story is worth telling: the entire anaerobic indication opened from an incidental observation of gingivitis clearing. Structurally the class is defined by the 5-position nitro group, which is the business end of the molecule — everything about mechanism and resistance turns on what happens to that nitro group.
:::

## Mechanism — An Inert Prodrug
<br>

- Metronidazole is an **inert prodrug**; its spectrum is set by the target's ability to **activate** it
- Four steps: **entry** (diffusion) → **reductive activation** (nitro group reduced to a cytotoxic **nitroso radical**) → **DNA damage** (strand breaks) → release of inactive end-products
- Reductive activation drives further uptake — a self-amplifying concentration gradient

::: aside
[@Edwards1993; @Dingsdag2018]
:::

::: {.notes}
This is the conceptual core of Part I. The drug does nothing until an organism reduces its nitro group; the short-lived nitroso radical then shreds DNA. Because activation itself pulls more drug into the cell, susceptible organisms accumulate the active form quickly. Everything downstream — selectivity, the resistance mechanisms, the protozoal activity — follows from this single fact.

<!-- IMAGE NEEDED: 5-nitroimidazole scaffold with the four-step activation pathway (entry, reduction to nitroso radical, DNA damage, inactive end-products). Redraw; source figures are copyrighted. -->
:::

## Why Selective for Anaerobes
<br>
...
::: aside
[@Lofmark2010; @LeCorn2007]
:::

::: {.notes}
The gram-positive-anaerobe gap is the highest-yield exam and clinical point of Part I. When someone reaches for metronidazole to "cover anaerobes," remind them it misses Actinomyces and Cutibacterium acnes entirely — so actinomycosis and C. acnes device infections are not metronidazole-treatable. This gap recurs in the lung-abscess story.
:::
...
::: aside
[@Kuriyama2011; @DiavCitrin2001]
:::

::: {.notes}
Short courses are very well tolerated; the neurotoxicity concern is a function of duration, so it surfaces in prolonged or repeated courses. Peripheral neuropathy may lag and can be slow to resolve. The pregnancy position is pragmatic: avoid first-trimester where possible, but the epidemiologic data are reassuring for later use when needed.
:::
...
::: aside
[@Holt2010; @Visapaa2002]
:::

::: {.notes}
If a trainee remembers one metronidazole interaction, make it warfarin — the INR can climb dangerously, and a pre-emptive dose reduction is the right move. Present the alcohol reaction honestly: it is a teaching staple with contested evidence; continue to advise abstinence, but do not overstate the certainty. Tinidazole and secnidazole are not CYP2E1 substrates and are unlikely to cause it.
:::

## Clinical Uses — Where It Still Wins
<br>
...
::: aside
[@Workowski2021; @Shih2022]
:::

::: {.notes}
Metronidazole still owns trichomoniasis and remains central to anaerobic and CNS anaerobic infection because of its penetration. The amebiasis teaching point: tissue amebicide plus a luminal agent to clear the cyst-passing state. The H. pylori paradox — in vitro resistance not predicting failure within combination therapy — is a nice reminder that susceptibility testing is not destiny.
:::
...
::: {.notes}
Pull the protozoal indications together. Trichomoniasis is the indication metronidazole still fully owns — note the shift to a 7-day regimen in women (better cure than single-dose), and always treat the partner. For giardiasis the single-dose nitroimidazoles and nitazoxanide have overtaken metronidazole on convenience and tolerability. The amebiasis rule — tissue amebicide plus luminal agent — is a perennial exam favourite.
:::
...
::: {.notes}
This is the classic "penetration is fine, spectrum is the problem" case. Lung abscesses are polymicrobial, and the streptococcal co-pathogens are simply outside metronidazole's activated-anaerobe niche. Never use it alone here.
:::
...
::: aside
[@Lofmark2010; @Soule2018; @Shah2021]
:::

::: {.notes}
Round out the clinical uses. In BV, all nitroimidazole routes work; clindamycin is often better tolerated. In surgical prophylaxis, metronidazole covers the anaerobic component and is a useful β-lactam-allergy alternative. On dosing: q8h has been traditional, but there is reasonable evidence that q12h is non-inferior for many indications, reducing pill burden — worth knowing for stewardship-minded prescribing.
:::
...
::: {.notes}
The practical differentiator among the nitroimidazoles is pharmacokinetics, not spectrum: longer half-lives enable single-dose therapy for the protozoal and BV indications and improve adherence. For serious anaerobic sepsis, though, stick with metronidazole — the evidence base for the newer agents in that high-stakes setting is limited.
:::
...
::: aside
[@Li2015; @Nelson2017; @Johnson2021]
:::

::: {.notes}
The demotion is the headline change in metronidazole practice. Why did it fail? Two pharmacologic reasons: it is almost completely absorbed in the upper gut, so luminal colonic concentrations are low and *fall as diarrhoea resolves* — exactly when you still need drug present — and resistance in C. difficile is emerging. Vancomycin and fidaxomicin, being non-absorbed, deliver reliably high luminal levels. This is the cleanest illustration of the session's theme.
:::
...
::: {.notes}
The recap slide for Part I. If the audience leaves with the prodrug mechanism, the two intrinsic-resistance traps, the CDI demotion-by-pharmacology, and the warfarin interaction, Part I has done its job. Now we pivot to agents where the pharmacology works *for* us — by concentrating in urine.
:::
...
::: {.notes}
This is Table 36.1 from the chapter (Gupta 2011). Two teaching points: fosfomycin's microbiologic cure (~80%) trails its clinical cure (~91%) and the other agents' (~88–94%), foreshadowing the Huttner trial; and fluoroquinolones, though effective, sit low on the list precisely because of collateral damage. TMP-SMX remains efficacious where local resistance is under ~20%.
:::
...
::: {.notes}
Like metronidazole, nitrofurantoin is a nitro-drug bioactivated intracellularly — but it hits several targets at once, which is why resistance has stayed low for 70 years. Know the intrinsic gaps: Proteus and Pseudomonas are out, and the Klebsiella/Enterobacter/Serratia group is unreliable. Its retained activity against VRE is an occasional niche use for VRE cystitis.
:::
...
## Fosfomycin — Mechanism and Spectrum
<br>
...
- Broad gram-negative activity incl. **>95% of ESBL *E. coli***; *E. faecalis*/*E. faecium* susceptible
...
::: {.notes}
A dedicated management slide because "salt and water overload" is the question that actually comes up on rounds. The message: this is a manageable but non-trivial toxicity that must be planned for, not discovered. In the frail, cardiac, or cirrhotic patient it can be the deciding factor against using IV fosfomycin at all. Pair the monitoring with proactive potassium and magnesium replacement, since the hypokalaemia is predictable from the sodium physiology.
:::
...
::: {.notes}
This answers the "occasionally MRSA" point directly. The Spanish BACSARM trial (Pujol 2021) paired fosfomycin with daptomycin for MRSA bacteraemia/endocarditis: the primary endpoint missed significance, but the combination clearly reduced microbiologic failure and complicated bacteraemia — at the cost of more adverse events (electrolyte/sodium, GI, some discontinuations). So it is a reasonable adjunct in persistent or complicated MRSA bacteraemia when you want to potentiate daptomycin, not a routine first move. Fosfomycin's cell-wall action synergises with daptomycin's membrane effect. Keep it framed as combination/salvage.
:::
...
::: aside
[@Gleckman1979; @Klinge1982]
:::

::: {.notes}
A brief orientation to the formulations, since the salt is not incidental — the hippuric or mandelic acid component nudges the urine toward the acidic pH the drug needs. Hiprex (hippurate) is the preparation behind the modern evidence base, including ALTAR, so it is the one to specify when prescribing.
:::
...
::: {.notes}
The ideal ALTAR patient: recurrent uncomplicated UTI, motivated to avoid chronic antibiotics, normal tract, no catheter. Methenamine hippurate is now a genuinely evidence-backed antibiotic-sparing choice for her. Pair it with counselling that it is prophylaxis only — breakthrough episodes still need a short treatment course — and with the intercourse-associated option if the history fits.
:::
...
::: {.notes}
Close by returning to the unifying theme and leaving the audience with the four practice-relevant anchors: the CDI demotion, the never-in-pyelonephritis rule for the urinary agents, the Huttner result, and the ALTAR-supported role for methenamine. These are the points most likely to change what they prescribe on Monday.
:::

## Discussion
<br>

- In your practice, where does single-dose fosfomycin still earn its place despite Huttner?
- How do you counsel and monitor for nitrofurantoin pulmonary toxicity in long-term prophylaxis?
- Would you offer methenamine hippurate to a recurrent-UTI patient wanting to avoid antibiotics?
- When is metronidazole still genuinely first-choice in 2026?

::: {.notes}
Discussion prompts pitched at specialists — each forces application of the pharmacology rather than recall. The fosfomycin question is a good one: convenience and ESBL coverage versus the Huttner efficacy gap. Use whichever fits the room.
:::
